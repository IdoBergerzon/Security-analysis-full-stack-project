...
import { g as accordionInjectionKey } from "./keys-BLeKMItg.mjs";
...
import { _ as _sfc_main$2 } from "./BCollapse.vue_vue_type_script_setup_true_lang-BL-ORxNX.mjs";
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    if (modelValue.value) {
      ...
      modelValue.value = (Array.isArray(parentData?.openItem.value) ? parentData?.openItem.value.includes(computedId.value) : parentData?.openItem.value === computedId.value) && !parentData?.initialAnimation.value;
    }
    ...
    const openInParent = computed(
      () => Array.isArray(parentData?.openItem.value) ? parentData?.openItem.value.includes(computedId.value) : parentData?.openItem.value === computedId.value
    );
    watch(
  ...
  setup(__props) {
    ...
    const sortAccordionItems = () => {
      ...
      if (modelValue.value) {
        ...
        modelValue.value = Array.isArray(index.value) ? index.value.map((idx) => accordionItems.value[idx]?.id) : accordionItems.value[index.value]?.id;
      }
    };
      ...
      (newValue, oldValue) => {
        ...
        if (!props.free) {
          ...
          if (Array.isArray(index.value)) {
            const newValue2 = index.value.map((item) => accordionItems.value[item]?.id);
            if (!areEqual(newValue2, modelValue.value)) {
              modelValue.value = newValue2;
            }
          } else {
            const newValue2 = accordionItems.value[index.value]?.id;
            if (newValue2 === void 0) {
              nextTick(() => {
                index.value = void 0;
              });
    ...
    watch(
      ...
      (newValue, oldValue) => {
        ...
        if (!props.free) {
          const idx = !Array.isArray(modelValue.value) ? accordionItems.value.findIndex((item) => item.id === modelValue.value) : accordionItems.value.findIndex((item) => item.id === modelValue.value?.[0]);
          if (idx !== -1) {
            if (index.value !== idx) {
              index.value = idx;
            }
...
//# sourceMappingURL=BAccordion.vue_vue_type_script_setup_true_lang-BSu5RTXu.mjs.map
